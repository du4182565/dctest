			...
			if (ipprototmp == "6") then
				...
				if (syn_flagstring == "1") then
					...
					if (ack_string == "1") then
						...
						if relenode == nil then
							...
							if relenode.seqtable[1] == 0 then
								if relenode.acktable[1] == 0 then
									relenode.seqtable[2] = seq_num
									relenode.acktable[2] = ack_num
									lastinfo.syn1 = lastinfo.syn1 - 1
								elseif(relenode.acktable[2] == ack_num)
								then
				...
				if(seq_num == 1)then
					if(ack_num == 1)then
						...
						--	add_reletable(reletable, hashindex, pinfo, seq_num, ack_num, pinfotimetmp, 0)
						--else
						--	if relenode.seqtable[2] == 1 then
						--		if relenode.acktable[2] == 0 then
						--			relenode.seqtable[3] = seq_num
						--			relenode.acktable[3] = ack_num
						--			lastinfo.syn2 = lastinfo.syn2 - 1
						--		elseif(relenode.acktable[3] == 1)
						--		then
						--			if (relenode.seqtable[3] == ack_num)then
						--				
						--			end
						--			timetable[pinfotimetmp].dumpsyn2 = timetable[pinfotimetmp].dumpsyn2 + 1
						--			lastdumpinfo.syn2 = lastdumpinfo.syn2 + 1
