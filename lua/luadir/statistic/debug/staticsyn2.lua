if (gui_enabled()) then
	...
	function upreletable(hashtable, relenode, seq_num, ack_num, timel, dir)
		local flag = 0
		local lenseq = 0
		local lenack = 0
		relenode.timel = timel
		if relenode ~= nil then
			for k, v  in pairs(relenode.seqtable) do
				if v == seq_num then
					flag = 1
				end
			end
			if flag == 1 then
				for k, v in pairs(relenode.acktable) do
					if v == ack_num then
						return 2
					end		
				end				
		...
		if flag == 0 then
			if relenode.seqtable ~= nil then
				lenseq = table.getn(relenode.seqtable)
				relenode.seqtable[lenseq + 1] = seq_num
			end
		end	
		if relenode.seqtable ~= nil then
			lenack = table.getn(relenode.acktable)
			relenode.acktable[lenack + 1] = ack_num		
		end
		return flag
	end	
				...
				if (syn_flagstring == "1") then
					if (ack_string == "0") then
						...
						if relenode == nil then
							...
							add_reletable(reletable, hashindex, pinfo, seq_num, ack_num, pinfotimetmp, 0)
						else
							timetable[pinfotimetmp].dumpsyn1 = timetable[pinfotimetmp].dumpsyn1 + 1
							local ret = upreletable(reletable, relenode, seq_num, ack_num, pinfotimetmp, 0)
							if ret == 2 then
								timetable[pinfotimetmp].syn1 = timetable[pinfotimetmp].syn1 + 1
								lastinfo.syn1 = lastinfo.syn1 + 1
